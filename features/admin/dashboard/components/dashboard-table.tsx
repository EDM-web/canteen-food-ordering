...
export default function DashboardTable({
  ...
  const [orders, setOrders] = useState<Order[]>(todayOrders);
  const [filterStatus, setFilterStatus] = useState<
    "ALL" | "AVAILABLE" | "UNAVAILABLE"
  >("ALL");

  // Keep state in sync when server props revalidate
  useEffect(() => {
    setOrders(todayOrders);
  }, [todayOrders]);

  // Subscribe to real-time order updates via Pusher
  useEffect(() => {
    const channel = pusherClient.subscribe("admin-orders");

    channel.bind("new-order", (newOrder: Order) => {
      setOrders((prevOrders) => {
        // ID ရှိပြီးသားလား စစ်ဆေးမည် (Duplicate ဖြစ်ခြင်းမှ ကာကွယ်ရန်)
        const isExist = prevOrders.some((order) => order.id === newOrder.id);
                  ...
                  >
                    <TableCell className="p-4 font-medium text-slate-800">
                      <div className="flex items-center gap-3">
                        ...
                        <Link
                          href={adminMenuDetailPath(item.id)}
                          className="font-normal text-slate-700 hover:text-orange-500 line-clamp-1 leading-loose"
                        >
                          {item.name}
                        </Link>
                      </div>
